...
class ProPublicaScraper:
    ...
    def get_organization_links(self, main_url):
        """
        Extract XML download links from organization's main page
        Returns: list_of_xml_urls
        """
        try:
            # Fetch the main page
    ...
    def _extract_financial_metrics_xml(self, tree):
        ...
        for category, elements in financial_elements.items():
            for element in elements:
                ...
                metrics[element] = value.text if value is not None else None
...
class ExcelOutputHandler:
    """Handles formatting and writing data to Excel in year-based sheets with metrics as rows and orgs as columns"""
    ...
    def consolidate_data(self, org_data):
        ...
        data_by_year = {year: {} for year in years_range}
                ...
                try:
                    ...
                    if tax_year != 'Unknown':
                        ...
                        if tax_year in years_range:
                            # Initialize org in this year if needed
                            if org_name not in data_by_year[tax_year]:
                                data_by_year[tax_year][org_name] = {}
                            ...
                            for display_col, field_name in self.field_mapping.items():
                                if display_col.startswith('Endowment '):
                                    ...
                                    if endowment_data and 'Year_0' in endowment_data:
                                        ...
                                        if value is not None:
                                            data_by_year[tax_year][org_name][display_col] = self.format_value(value, display_col)
                                        else:
                                            data_by_year[tax_year][org_name][display_col] = None
                                    else:
                                        data_by_year[tax_year][org_name][display_col] = None
                                else:
        ...
        # Now convert to DataFrames (one per year)
        year_dfs = {}
        for year in years_range:
            if not data_by_year[year]:
                # Create empty DataFrame with metrics if no data for this year
                df = pd.DataFrame({'Metric': list(self.field_mapping.keys())})
            else:
                # Create rows for each metric
                rows = []
                for metric_name in self.field_mapping.keys():
                    row = {'Metric': metric_name}
                    # Add data for each organization
                    for org_name in data_by_year[year].keys():
                        row[org_name] = data_by_year[year][org_name].get(metric_name, None)
                    rows.append(row)
    ...
    def write_to_excel(self, year_dfs):
        """Write data to Excel with each year in its own sheet"""
        try:
            if not year_dfs:
    ...
    def _format_worksheet(self, worksheet, df):
        ...
        for idx, col in enumerate(df.columns):
            ...
            # Format organization columns (skip first column which is Metric)
            if idx > 0:  # Skip the "Metric" column
...
def main():
    ...
    extractor = FinancialDataExtractor()
    excel_handler = ExcelOutputHandler(r'C:\Users\aronc\OneDrive\Documents\YearSheets.xlsx')

    # Dictionary to store all org data
    all_org_data = {}
    ...
    while True:
        ...
        try:
            # Get XML links
            xml_links = parser.scraper.get_organization_links(org_url)
            ...
            for url in xml_links:
                try:
                    ...
                    all_org_data[org_name].append(result)
    ...
    if all_org_data:
        year_dfs = excel_handler.consolidate_data(all_org_data)
        if excel_handler.write_to_excel(year_dfs):
